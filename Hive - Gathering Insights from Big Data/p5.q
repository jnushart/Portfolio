...
CREATE VIEW filtered AS SELECT id, sum(errors) AS errors FROM fielding GROUP BY id;
SELECT subquery.id FROM (SELECT id, DENSE_RANK() OVER (ORDER BY errors DESC) AS rank FROM filtered) subquery WHERE rank = 1;